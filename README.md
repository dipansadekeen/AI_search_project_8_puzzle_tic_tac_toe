...
| Dijkstra | 5,085 | 14 |
| A\* (Manhattan) | 59 | 14 |
| Greedy Best-First | varies (not optimal) | ≥14 |
| IDA\* | ~few hundred | 14 (optimal) |
...
goal. IDA\* finds the same optimal-length path with far less memory than A\*.
Greedy Best-First typically expands even fewer nodes but produces a longer path.
...
(score 0), as expected for perfect play. On 4×4 boards full minimax is
computationally infeasible; depth-limited Alpha-Beta with a heuristic evaluation
function makes the problem tractable while still playing strong moves.
